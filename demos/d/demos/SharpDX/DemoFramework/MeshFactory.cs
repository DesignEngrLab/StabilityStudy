...
namespace DemoFramework
{
    ...
    public class ShapeData : System.IDisposable
    {
        ...
        public void SetVertexBuffer(Device device, BulletSharp.Math.Vector3[] vectors)
        {
    ...
    public class MeshFactory : System.IDisposable
    {
        ...
        public static BulletSharp.Math.Vector3 GetVectorByAxis(float x, float y, float z, int axis)
        {
            switch (axis)
            {
                case 0:
                    return new BulletSharp.Math.Vector3(y, z, x);
                case 1:
                    return new BulletSharp.Math.Vector3(x, y, z);
                default:
                    return new BulletSharp.Math.Vector3(z, x, y);
            }
        }

        ShapeData CreateBoxShape(BoxShape shape)
        {
         var size = shape.HalfExtentsWithMargin;
            float x = size.X;
            float y = size.Y;
            float z = size.Z;
            ...
            var vertices = new BulletSharp.Math.Vector3[shapeData.VertexCount * 2];
        ...
        public ShapeData CreateCapsule(CapsuleShape shape)
        {
           ...
           var vertices = new BulletSharp.Math.Vector3[shapeData.VertexCount * 2];
            ...
            vertices[v++] = GetVectorByAxis(-BulletSharp.Math.Vector3.UnitY, up);
            vertices[v++] = GetVectorByAxis(0, cylinderHalfHeight + radius, 0, up);
            vertices[v++] = GetVectorByAxis(BulletSharp.Math.Vector3.UnitY, up);
            ...
            BulletSharp.Math.Vector3 vTemp;
            BulletSharp.Math.Vector3 cylinderOffset = GetVectorByAxis(0, -cylinderHalfHeight, 0, up);
            for (j = 0; j < stacks - 1; j++)
            {
                ...
                for (k = 0; k < slices; k++)
                {
                    ...
                    vertices[v++] = BulletSharp.Math.Vector3.Normalize(vTemp);
                }
            }
        ...
        */
        ...
        BulletSharp.Math.Vector3 GetVectorByAxis(BulletSharp.Math.Vector3 vector, int axis)
        {
            switch (axis)
            {
                case 0:
                    return new BulletSharp.Math.Vector3(vector.Y, vector.Z, vector.X);
                case 1:
                    return vector;
                default:
                    return new BulletSharp.Math.Vector3(vector.Z, vector.X, vector.Y);
            }
        }
        ...
        ShapeData CreateCylinderShape(CylinderShape shape)
        {
           ...
           var vertices = new BulletSharp.Math.Vector3[shapeData.VertexCount * 2];
            ...
            {
                normal = GetVectorByAxis(side * BulletSharp.Math.Vector3.UnitY, up);

                baseIndex = index;
                vertices[v++] = GetVectorByAxis(new BulletSharp.Math.Vector3(0, side * halfHeight, 0), up);
                vertices[v++] = normal;

                vertices[v++] = GetVectorByAxis(new BulletSharp.Math.Vector3(0, side * halfHeight, radius), up);
                vertices[v++] = normal;
                index += 2;
                ...
                for (int j = 1; j < numSteps; j++)
                {
                    ...
                    vertices[v++] = GetVectorByAxis(new BulletSharp.Math.Vector3(x, side * halfHeight, z), up);
                    vertices[v++] = normal;
            ...
            normal = GetVectorByAxis(new BulletSharp.Math.Vector3(0, 0, radius), up);
            normal.Normalize();

            baseIndex = index;
            vertices[v++] = GetVectorByAxis(new BulletSharp.Math.Vector3(0, halfHeight, radius), up);
            vertices[v++] = normal;
            ...
            for (int j = 1; j < numSteps + 1; j++)
            {
                ...
                normal = GetVectorByAxis(new BulletSharp.Math.Vector3(x, 0, z), up);
                normal.Normalize();

                vertices[v++] = GetVectorByAxis(new BulletSharp.Math.Vector3(x, halfHeight, z), up);
                vertices[v++] = normal;
        ...
        ShapeData CreateConvexHullShape(ConvexHullShape shape)
        {
           ...
           var vertices = new BulletSharp.Math.Vector3[indexCount * 2];

            int v = 0, i;
            for (i = 0; i < indexCount; i += 3)
            {
                BulletSharp.Math.Vector3 v0 = points[(int)indices[i]];
                BulletSharp.Math.Vector3 v1 = points[(int)indices[i + 1]];
                BulletSharp.Math.Vector3 v2 = points[(int)indices[i + 2]];

                var v01 = v0 - v1;
                var v02 = v0 - v2;
                var normal = BulletSharp.Math.Vector3.Cross(v01, v02);
                normal.Normalize();
        ...
        */
        ShapeData CreateSphereShape(SphereShape shape)
        {
            ...
            // Vertices
            // Top and bottom
            vertices[v++] = new BulletSharp.Math.Vector3(0, -radius, 0);
            vertices[v++] = -BulletSharp.Math.Vector3.UnitY;
            vertices[v++] = new BulletSharp.Math.Vector3(0, radius, 0);
            vertices[v++] = BulletSharp.Math.Vector3.UnitY;
            ...
            for (j = 0; j < stacks - 1; j++)
            {
                ...
                for (k = 0; k < slices; k++)
                {
                    ...
                    vTemp = new BulletSharp.Math.Vector3((float)Math.Cos(vAngle) * (float)Math.Sin(angle), (float)Math.Sin(vAngle), (float)Math.Cos(vAngle) * (float)Math.Cos(angle));
        ...
        */
        ...
        ShapeData CreateTriangleMeshShape(StridingMeshInterface meshInterface)
        {
           ...
           var vertices = new BulletSharp.Math.Vector3[shapeData.VertexCount * 2];
            ...
            {
                var v0 = vertexStream.Read<Vector3>();
                vertexStream.Position += vStrideExtra;
                var v1 = vertexStream.Read<Vector3>();
                vertexStream.Position += vStrideExtra;
                var v2 = vertexStream.Read<Vector3>();
                vertexStream.Position += vStrideExtra;
        ...
        void InitRigidBodyInstance(CollisionObject colObj, CollisionShape shape, ref BulletSharp.Math.Matrix  transform)
        {
            if (shape.ShapeType == BroadphaseNativeType.CompoundShape)
            {
                ...
                {
                   var childTransform = child.Transform * transform;
        ...
        void InitSoftBodyInstance(SoftBody softBody, CollisionShape shape)
        {
            ...
            shapeData.Instances.Add(new InstanceData()
            {
                WorldTransform = BulletSharp.Math.Matrix.Identity,
                Color = softBodyColor
            });
